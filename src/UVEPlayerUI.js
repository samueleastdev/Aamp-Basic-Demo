...
function toggleCC() {
  if (ccStatus === false) {
    ...
    ccStatus = false;
    document.getElementById("ccIcon").src =
      "../icons/closedCaptioningDisabled.png";
  }
}
...
function toggleOverlay() {
  ...
  document.getElementById("logCheck").checked =
    !document.getElementById("logCheck").checked;
...
var HTML5PlayerControls = function () {
  ...
  this.keyUp = function () {
    if (this.dropDownListVisible) {
      ...
      this.components[this.currentPos] == this.playButton ||
      this.components[this.currentPos] == this.rwdButton ||
      this.components[this.currentPos] == this.skipBwdButton ||
      this.components[this.currentPos] == this.skipFwdButton ||
      this.components[this.currentPos] == this.fwdButton ||
      this.components[this.currentPos] == this.muteButton ||
      this.components[this.currentPos] == this.ccButton
    ) {
  ...
  this.keyDown = function () {
    if (this.dropDownListVisible) {
      ...
      this.components[this.currentPos] == this.videoFileList ||
      this.components[this.currentPos] == this.autoVideoLogButton ||
      this.components[this.currentPos] == this.homeContentButton
    ) {
  ...
  this.keyEventHandler = function (e, type) {
    ...
    console.log("UVE TEST Pressed keycode" + keyCode);
    e.preventDefault();
    if (type == "keydown") {
      switch (keyCode) {
        case 37: // Left Arrow
...
function showTrickmodeOverlay(speed) {
  ...
  if (speed > 0) {
    ...
    document.getElementById("ffModal").style["-webkit-transform"] =
      "scaleX(-1)";
  }
